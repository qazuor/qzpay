...
interface CatalogState {
  ...
  addAddon: (addon: Partial<QZPayAddOn> & { name: string; unitAmount: number; currency: string; billingInterval: QZPayBillingInterval | 'one_time' }) => QZPayAddOn;
...
export const useCatalogStore = create<CatalogState>()((set, get) => ({
  ...
  updateLimitDefinition: (key, updates) => {
    ...
    if (data.limitDefinitions[key]) {
      ...
      importPlaygroundData(data);
      set({
        limitDefinitions: get().limitDefinitions.map(l =>
          l.key === key ? { ...l, ...updates, updatedAt: new Date() } : l
        ),
      });
    }
  },
