...
export function CustomerPanel() {
    ...
    try {
      ...
      const customerInfoList: CustomerInfo[] = customerList.map((customer) => {
        ...
        const subscription = subscriptions.find(
          (s) => s.customerId === customer.id && s.status !== 'canceled'
        ) || null;
        ...
        if (subscription) {
          ...
          const lifecycleState = (subscription.metadata?.lifecycleState as Record<string, unknown>) || {};
          ...
          if (subscription.currentPeriodEnd) {
            ...
            if (inGracePeriod) {
              ...
              const graceDaysRemaining = Math.ceil((gracePeriodEnd.getTime() - currentTime.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const handleIncrementLimit = async (customerId: string, limitKey: string, amount: number = 1) => {
    if (!billing) return;
    try {
  ...
  const handleResetLimit = async (customerId: string, limitKey: string, maxValue: number) => {
    if (!billing) return;
    try {
          ...
          customers.map((info) => (
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {/* Status Badge */}
                <div className={`flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium flex-shrink-0 ${getStatusColor(info.subscriptionStatus, info.inGracePeriod)}`}>
              ...
              {info.subscriptionStatus !== 'none' && (
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="flex items-center gap-1.5" style={{ color: 'var(--color-text-secondary)' }}>
                    ...
                    <span>{formatAmount(info.priceAmount, info.priceCurrency)}/{info.billingInterval}</span>
                  </div>
                  {/* Time Remaining */}
                  <div className={`flex items-center gap-1.5 ${getTimeRemainingColor(info.daysRemaining, info.subscriptionStatus, info.inGracePeriod)}`}>
                    <Calendar className="h-3 w-3 flex-shrink-0" />
                    <span>{info.timeRemaining}{!info.inGracePeriod && info.daysRemaining >= 0 ? ' left' : ''}</span>
                  </div>
                  {/* Payment Method */}
                  <div className="flex items-center gap-1.5" style={{ color: info.hasPaymentMethod ? 'var(--color-text-secondary)' : 'var(--color-text-muted)' }}>
                    <CreditCard className="h-3 w-3 flex-shrink-0" />
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {info.entitlements.map((key) => (
                      <span
                        key={key}
                        className="px-1.5 py-0.5 text-xs rounded bg-purple-900/40 text-purple-300"
                      >
                        {key}
                      </span>
                    ))}
                      ...
                      return (
                        <div key={limit.key}>
                          <div className="flex items-center justify-between text-xs mb-0.5">
                            <span style={{ color: 'var(--color-text-muted)' }}>{limit.key}</span>
                            <div className="flex items-center gap-1">
                              <span className={isAtLimit ? 'text-red-400' : isNearLimit ? 'text-yellow-400' : 'text-green-400'}>
              ...
              {info.lastPayment && (
                <div className="flex items-center justify-between text-xs pt-1 border-t" style={{ borderColor: 'var(--color-border)' }}>
                  ...
                  <span style={{ color: 'var(--color-text-secondary)' }}>
                    ...
                    <span className={info.lastPayment.status === 'succeeded' ? 'text-green-400 ml-1' : 'text-red-400 ml-1'}>
