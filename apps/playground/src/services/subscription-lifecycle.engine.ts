...
import { processPaymentInBackend } from '../lib/backend-client';
...
export interface LifecycleConfig {
    ...
    // Backend configuration for real payments
    backend?: {
        isConnected: boolean;
        paymentMode: 'stripe' | 'mercadopago' | 'mock';
        sessionId: string;
    };
}
...
async function processTrialing(
    ...
    const daysUntilTrialEnd = Math.ceil((trialEnd.getTime() - currentTime.getTime()) / (1000 * 60 * 60 * 24));
...
async function processActive(
    ...
    const daysUntilRenewal = Math.ceil((currentPeriodEnd.getTime() - currentTime.getTime()) / (1000 * 60 * 60 * 24));
...
async function processGracePeriod(
    ...
    const daysSinceGracePeriodStart = Math.floor((currentTime.getTime() - gracePeriodStart.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const daysSinceLastAttempt = Math.floor((currentTime.getTime() - lastAttempt.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const daysUntilGracePeriodEnd = Math.ceil((gracePeriodEnd.getTime() - currentTime.getTime()) / (1000 * 60 * 60 * 24));
...
async function attemptPayment(
    ...
    try {
        ...
        ) as Array<{ id: string; metadata?: { stripePaymentMethodId?: string } }>;
        ...
        const paymentMethodCollectedInSubscription = subscriptionMetadata?.paymentMethodCollected === true;

        const hasPaymentMethod = customerPaymentMethods.length > 0 || paymentMethodCollectedInSubscription;

        // If backend is connected and we have a real payment method, use the backend
        if (config.backend?.isConnected && hasPaymentMethod && config.backend.paymentMode !== 'mock') {
            console.log('[Lifecycle] Using backend for real payment processing');

            // Find the payment method with Stripe/MP ID
            const paymentMethodWithProviderId = customerPaymentMethods.find((pm) => pm.metadata?.stripePaymentMethodId);
            const stripePaymentMethodId = paymentMethodWithProviderId?.metadata?.stripePaymentMethodId;

            // Get customer data
            const customer = await billing.customers.get(subscription.customerId);

            if (stripePaymentMethodId && customer) {
                console.log('[Lifecycle] Processing payment with Stripe PaymentMethod:', stripePaymentMethodId);

                const backendResult = await processPaymentInBackend(config.backend.sessionId, {
                    customerId: subscription.customerId,
                ...
                console.log('[Lifecycle] Backend payment result:', backendResult);

                const isSuccess = backendResult.success && backendResult.data?.status === 'succeeded';

                // Record the payment locally
                if (backendResult.data?.id) {
                    await billing.payments.record({
                        ...
                        provider: config.backend.paymentMode,
                        metadata: {
                            subscriptionId: subscription.id,
                            type: 'subscription_renewal',
                            automated: true
                ...
                addEvent({
                    ...
                    payload: {
                        ...
                        failureReason: !isSuccess ? backendResult.error || 'payment_failed' : undefined
                    },
                    timestamp: currentTime
                });
        ...
        const isSuccess = hasPaymentMethod ? Math.random() < config.paymentSuccessRate : false;
        ...
        addEvent({
            ...
            payload: {
                ...
                simulated: true,
                hasPaymentMethod,
                failureReason: !isSuccess ? (hasPaymentMethod ? 'payment_declined' : 'no_payment_method') : undefined
            },
            timestamp: currentTime
        });
 ...
 */
async function updateLifecycleState(billing: QZPayBilling, subscriptionId: string, state: SubscriptionLifecycleState): Promise<void> {
    try {
        ...
        if (subscription) {
            await billing.subscriptions.update(subscriptionId, {
                metadata: {
                    ...
                    // Serialize state to JSON string since metadata values must be primitives
