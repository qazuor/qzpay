...
import { AlertCircle, CheckCircle, UserX, XCircle } from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            <div className="card p-4">
                <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                    <strong>{t('demos.limitGate.currentPlan')}:</strong> {customerPlan?.name || t('demos.limitGate.noPlan')}
                </p>
                    ...
                    {allLimitKeys.map((limitKey) => {
                        ...
                        return (
                            <div key={limitKey} className="border rounded-lg p-4" style={{ borderColor: 'var(--color-border)' }}>
                                ...
                                {isWithinLimit ? (
                                    <p className="text-sm text-green-400">{t('demos.limitGate.withinLimit')}</p>
                                ) : (
                                    <p className="text-sm text-red-400">{t('demos.limitGate.exceeded')}</p>
                                )}
                            </div>
                        );
                    })}
