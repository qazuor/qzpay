...
import { AlertCircle, UserX, Lock, Unlock } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="card p-4">
        <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
          <strong>{t('demos.entitlementGate.currentPlan')}:</strong>{' '}
          {customerPlan?.name || t('demos.entitlementGate.noPlan')}
        </p>
        {customerEntitlements.length > 0 && (
          <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>
            <strong>{t('demos.entitlementGate.activeEntitlements')}:</strong>{' '}
            {customerEntitlements.join(', ')}
          </p>
        )}
      </div>
              ...
              <div key={entitlementKey} className="border rounded-lg p-4" style={{ borderColor: 'var(--color-border)' }}>
                ...
                {hasAccess ? (
                  <p className="text-sm text-green-400">
                    {t('demos.entitlementGate.unlocked')}
                  </p>
                ) : (
                  <p className="text-sm text-red-400">
                    {t('demos.entitlementGate.locked')}
                  </p>
                )}
              </div>
            );
