...
import type { QZPayPayment, QZPayCustomer } from '@qazuor/qzpay-core';
...
export function PaymentsView() {
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [paymentsResult, custResult] = await Promise.all([
    ...
    <div className="max-w-4xl space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          tips={[
            t('payments.tips.tip1'),
            t('payments.tips.tip2'),
            t('payments.tips.tip3'),
          ]}
        />
      ) : (
            ...
            <form onSubmit={handleCreatePayment}>
              <div className="modal-body space-y-4">
                ...
                <div>
                  <label htmlFor="paymentDescription" className="label">
                    {t('payments.oneTime.descriptionLabel', 'Description')} <span className="text-gray-500">({t('payments.oneTime.optional', 'optional')})</span>
                  </label>
                  <input
                  ...
                  <div className="p-3 rounded-lg border" style={{ backgroundColor: 'var(--color-surface-elevated)', borderColor: 'var(--color-border)' }}>
                    <div className="flex justify-between items-center font-semibold">
                      ...
                      <span style={{ color: 'var(--color-accent)' }}>
                        {new Intl.NumberFormat('en-US', { style: 'currency', currency: formData.currency }).format(parseFloat(formData.amount))}
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="modal-footer">
                ...
                <button
                  type="submit"
                  className="btn btn-primary"
                  disabled={!formData.customerId || !formData.amount || parseFloat(formData.amount) <= 0}
                >
