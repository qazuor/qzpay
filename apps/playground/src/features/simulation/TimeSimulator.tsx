import { ChevronDown, ChevronUp, Clock, FastForward, Pause, Play, RotateCcw, Square, Timer } from 'lucide-react';
/**
...
import { DEFAULT_LIFECYCLE_CONFIG, processSubscriptionLifecycle } from '../../services/subscription-lifecycle.engine';
...
export function TimeSimulator({ onTimeAdvanced, compact = false }: TimeSimulatorProps) {
    ...
    const runLifecycleProcessing = useCallback(async () => {
        ...
        try {
            ...
            // Build lifecycle config with backend info for real payments
            const lifecycleConfig = {
                ...DEFAULT_LIFECYCLE_CONFIG,
                backend:
                    isBackendConnected && paymentMode !== 'mock'
                        ? {
                              isConnected: true,
                              paymentMode: paymentMode as 'stripe' | 'mercadopago' | 'mock',
                              sessionId: backendSessionId || ''
                          }
                        : undefined
            };

            await processSubscriptionLifecycle(billing, newTime, lifecycleConfig);
        } catch (error) {
            console.error('Error processing lifecycle:', error);
        } finally {
            setIsProcessing(false);
        }
    }, [billing, isInitialized, timeSimulation.enabled, getCurrentTime, isBackendConnected, paymentMode, backendSessionId]);
    ...
    const daysDiff =
        timeSimulation.enabled && timeSimulation.simulatedDate
    ...
    if (compact) {
        return (
            <div className="relative flex items-center gap-1" ref={dropdownRef}>
                ...
                >
                    {isAutoPlaying ? <Timer className="h-4 w-4 animate-spin" /> : <Clock className="h-4 w-4" />}
                    {timeSimulation.enabled ? (
                ...
                <div className="p-3 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="p-2 rounded-lg bg-black/20">
                            <div className="text-xs text-gray-400 mb-1">{t('timeSimulator.simulatedTime', 'Simulated Time')}</div>
                            <div className="font-mono" style={{ color: timeSimulation.enabled ? '#a855f7' : 'var(--color-text)' }}>
                                {formatDate(currentTime)}
                            </div>
                            <div className="text-xs font-mono text-gray-500">{formatTime(currentTime)}</div>
                        </div>

                        {/* Real Time */}
                        <div className="p-2 rounded-lg bg-black/20">
                            <div className="text-xs text-gray-400 mb-1">{t('timeSimulator.realTime', 'Real Time')}</div>
                            <div className="font-mono text-gray-400">{formatDate(realTime)}</div>
                            <div className="text-xs font-mono text-gray-500">{formatTime(realTime)}</div>
                        </div>
                    </div>
                    ...
                    {timeSimulation.enabled && daysDiff !== 0 && (
                        <div
                            className={`text-center py-2 px-3 rounded-lg border ${
                                isAutoPlaying ? 'bg-green-500/10 border-green-500/20' : 'bg-purple-500/10 border-purple-500/20'
                            }`}
                        >
                            <span className={`text-sm ${isAutoPlaying ? 'text-green-300' : 'text-purple-300'}`}>
                                {daysDiff > 0 ? '+' : ''}
                                {daysDiff} {t('timeSimulator.daysFromNow', 'days from now')}
                            </span>
                        </div>
                    )}
                    ...
                    {timeSimulation.enabled && (
                        <div
                            className={`p-3 rounded-lg border ${
                                isAutoPlaying ? 'bg-green-500/10 border-green-500/30' : 'bg-black/20 border-transparent'
                            }`}
                        >
                            ...
                            <div className="mt-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        max="10"
                                        value={autoPlaySpeed}
                                        onChange={(e) => setAutoPlaySpeed(Number.parseInt(e.target.value, 10))}
                    ...
                    {timeSimulation.enabled && !isAutoPlaying && (
                        <>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleAdvanceTime(Number.parseInt(customDays, 10) || 0)}
                                    className="btn btn-primary py-1.5 text-sm"
                                    disabled={!customDays || Number.parseInt(customDays, 10) <= 0}
                                >
                                    <FastForward className="h-3 w-3" />
                    ...
                    <p className="text-xs text-gray-500">
                        {isAutoPlaying
                            ? t(
                                  'timeSimulator.helpAutoPlay',
                                  'Time is advancing automatically. Watch how subscriptions renew, expire, and billing cycles progress.'
                              )
                            : timeSimulation.enabled
                              ? t(
                                    'timeSimulator.helpEnabled',
                                    'Advance time to test subscription renewals, trial expirations, and billing cycles.'
                                )
