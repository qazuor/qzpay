import type { QZPayCustomer, QZPayPrice, QZPaySubscription } from '@qazuor/qzpay-core';
import {
    AlertTriangle,
    ArrowRight,
    ArrowUpDown,
...
export function SubscriptionsView() {
    ...
    const handleCancel = async (id: string) => {
        ...
        const confirmed = await confirm.show({
            ...
            variant: 'danger',
            confirmText: tc('buttons.confirm')
        });
        ...
        try {
            ...
            loadData();
            toast.success(t('subscriptions.canceled', 'Subscription canceled successfully'));
        } catch (error) {
    ...
    const handleRenewalComplete = async (result: PaymentResult) => {
        ...
        try {
            if (result.status === 'succeeded') {
                ...
                loadData();
                toast.success(t('subscriptions.renewed', 'Subscription renewed successfully'));
            } else {
                toast.error(result.error || t('subscriptions.renewal.failed', 'Renewal payment failed'));
            }
        } catch (error) {
    ...
    const handleCreateSubscription = async (e: React.FormEvent) => {
        ...
        const existingSubscription = subscriptions.find(
            (sub) => sub.customerId === formData.customerId && (sub.status === 'active' || sub.status === 'trialing')
        );
        if (existingSubscription) {
            toast.warning(
                t(
                    'subscriptions.errors.alreadyHasSubscription',
        ...
        if (hasTrial && !formData.requireCardForTrial) {
            try {
                ...
                setFormData({ customerId: '', priceId: '', requireCardForTrial: false, promoCode: '' });
                setPromoValidation(null);
    ...
    const handlePaymentComplete = async (result: PaymentResult) => {
        ...
        try {
            if (result.status === 'succeeded') {
                ...
                toast.error(result.error || t('subscriptions.payment.failed'));
            }
        } catch (error) {
            toast.error(error instanceof Error ? error.message : 'Failed to create subscription');
        } finally {
        ...
        <div className="max-w-4xl space-y-6">
            <PageHeader
                ...
                helpContent={
                    <div className="space-y-2">
                        <p>{t('subscriptions.helpContent')}</p>
                ...
                actions={
                    <div className="flex items-center gap-2">
                        {customers.length > 0 && (
                            <button type="button" onClick={() => setOneTimePaymentModalOpen(true)} className="btn btn-secondary">
                ...
                >
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-sm mb-3" style={{ color: 'var(--color-text-secondary)' }}>
                            {t(
                                'subscriptions.renewal.pendingDescription',
                                'These subscriptions have passed their billing period and need to be renewed.'
                            )}
                        </p>
                    ...
                    <div>
                        ...
                        <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                            ...
                            {customers.length === 0 && (
                                <span
                                    className="block"
                                    dangerouslySetInnerHTML={{ __html: `• ${t('subscriptions.prerequisitesNeeded.needCustomer')}` }}
                                />
                            )}
                            {prices.length === 0 && (
                                <span
                                    className="block"
                                    dangerouslySetInnerHTML={{ __html: `• ${t('subscriptions.prerequisitesNeeded.needPrice')}` }}
                                />
                            )}
                        </p>
                    </div>
            ...
            {isLoading ? (
                ...
                <EmptyState
                    ...
                    tips={[t('subscriptions.tips.tip1'), t('subscriptions.tips.tip2'), t('subscriptions.tips.tip3')]}
                />
            ) : (
                    ...
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleCreateSubscription}>
                            <div className="modal-body space-y-4">
                                ...
                                <div>
                                    <label htmlFor="promoCode" className="label">
                                        {t('subscriptions.modal.promoCodeLabel', 'Promo Code')}{' '}
                                        <span className="text-gray-500">({t('subscriptions.modal.optional', 'optional')})</span>
                                    </label>
                                    <div className="flex gap-2">
                                        ...
                                        >
                                            {isValidatingPromo
                                                ? t('subscriptions.modal.validating', 'Validating...')
                                                : t('subscriptions.modal.apply', 'Apply')}
                                        </button>
                                    </div>
                                    {promoValidation && (
                                        <div
                                            className={`mt-2 p-2 rounded text-sm ${promoValidation.valid ? 'bg-green-900/30 text-green-300 border border-green-500/30' : 'bg-red-900/30 text-red-300 border border-red-500/30'}`}
                                        >
                                            {promoValidation.valid ? (
                                                <span>
                                                    {promoValidation.discountPercent
                                                        ? t('subscriptions.modal.discountApplied', {
                                                              discount: `${promoValidation.discountPercent}%`
                                                          })
                                                        : t('subscriptions.modal.discountApplied', {
                                                              discount: new Intl.NumberFormat('en-US', {
                                                                  style: 'currency',
                                                                  currency: selectedPrice?.currency || 'USD'
                                                              }).format((promoValidation.discountAmount || 0) / 100)
                                                          })}
                                                </span>
                                            ) : (
                                    ...
                                    >
                                        <div className="flex justify-between items-center">
                                            <span style={{ color: 'var(--color-text-secondary)' }}>
                                                {t('subscriptions.modal.originalPrice', 'Original Price')}
                                            </span>
                                            <span
                                                className={promoValidation?.valid ? 'line-through text-gray-500' : ''}
                                                style={{ color: promoValidation?.valid ? undefined : 'var(--color-text)' }}
                                            >
                                                {new Intl.NumberFormat('en-US', {
                                                    style: 'currency',
                                                    currency: selectedPrice.currency
                                                }).format(selectedPrice.unitAmount / 100)}
                                            </span>
                                        </div>
                                        {promoValidation?.valid && (
                                            <>
                                                ...
                                                <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-700 font-semibold">
                                                    <span style={{ color: 'var(--color-text)' }}>
                                                        {t('subscriptions.modal.totalPrice', 'Total')}
                                                    </span>
                                                    <span style={{ color: 'var(--color-accent)' }}>
                                                        {new Intl.NumberFormat('en-US', {
                                                            style: 'currency',
                                                            currency: selectedPrice.currency
                                                        }).format(calculateDiscountedAmount(selectedPrice.unitAmount) / 100)}
                                                    </span>
                                                </div>
                                            </>
                                        )}
                                ...
                                {hasTrial && (
                                    ...
                                    >
                                        <input
                                            ...
                                            onChange={(e) => setFormData((prev) => ({ ...prev, requireCardForTrial: e.target.checked }))}
                                ...
                                <div className="p-3 rounded-lg text-sm" style={{ backgroundColor: 'var(--color-surface-elevated)' }}>
                                            ...
                                            <>
                                                <li
                                                    dangerouslySetInnerHTML={{
                                                        __html: `• ${t('subscriptions.modal.info.trialStart', { days: selectedPrice?.trialDays })}`
                                                    }}
                                                />
                                                <li
                                                    dangerouslySetInnerHTML={{ __html: `• ${t('subscriptions.modal.info.noPaymentNow')}` }}
                                                />
                                            </>
                                        ) : (
                                            <>
                            ...
                            <div className="modal-footer">
                                ...
                                <button type="submit" className="btn btn-primary" disabled={!formData.customerId || !formData.priceId}>
                                    {hasTrial && !formData.requireCardForTrial
