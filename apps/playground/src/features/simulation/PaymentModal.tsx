import { AlertTriangle, CheckCircle, ChevronDown, CreditCard, Loader2, Server, X, XCircle } from 'lucide-react';
...
import { getCustomerCards, processPaymentInBackend } from '../../lib/backend-client';
import type { PaymentMode } from '../../lib/billing';
import { useConfigStore } from '../../stores/config.store';

// Unified saved card type
interface SavedCard {
    id: string;
    customerId: string;
...
// Type guard to check if card has MercadoPago fields
function isMercadoPagoCard(card: SavedCard): card is SavedCard & Required<Pick<SavedCard, 'paymentMethodId' | 'paymentMethodName'>> {
...
export function PaymentModal({
    ...
    const [triggerStripePaymentMethod, setTriggerStripePaymentMethod] = useState(false);
    const [stripePaymentMethodId, setStripePaymentMethodId] = useState<string | null>(null);
    const [stripeClientSecret, setStripeClientSecret] = useState<string | null>(null);

    // Unified saved cards state
    ...
    const useStripeCardForm = isStripe && isBackendConnected;
    ...
    const getCardNumber = (): string => {
        ...
        const index = Number.parseInt(selectedCard, 10);
        return testCards[index]?.value || testCards[0].value;
    };

    // Load saved cards when modal opens (unified for all payment modes)
    useEffect(() => {
        const loadSavedCards = async () => {
            if (!isOpen || !customerId) return;

            // For Stripe with backend, don't load saved cards - StripeCardForm handles it
            if (useStripeCardForm) {
                setSavedCards([]);
            ...
            try {
                ...
                if (useMPCardForm) {
                    ...
                    if (response.success && response.data?.cards) {
                        // Map backend cards to unified SavedCard
                        const cards: SavedCard[] = response.data.cards.map((card) => ({
                            id: card.id,
                            customerId: customerId, // Use the function parameter
                            ...
                            paymentMethodThumbnail: card.paymentMethodThumbnail
                        }));
                        setSavedCards(cards);
                    ...
                    // For mock mode, load from local storage
                    ...
                    // Filter and map cards for this customer
                    const cards: SavedCard[] = paymentMethods
        ...
        loadSavedCards();
    }, [isOpen, useMPCardForm, useStripeCardForm, customerId, backendSessionId, getBackendSessionId]);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            // Reset Stripe state
            setTriggerStripePaymentMethod(false);
    ...
    // Stripe handlers
    const handleStripePaymentMethodCreated = useCallback((paymentMethodId: string) => {
        setStripePaymentMethodId(paymentMethodId);
    }, []);

    const handleStripePaymentMethodComplete = useCallback(() => {
        setTriggerStripePaymentMethod(false);
    }, []);

    const handleStripePaymentConfirmed = useCallback(
        (status: 'succeeded' | 'failed', error?: string) => {
            setStripeClientSecret(null);
            if (status === 'succeeded') {
                const paymentResult: PaymentResult = { status: 'succeeded' };
                setResult(paymentResult);
                setTimeout(() => onPaymentComplete(paymentResult), 1500);
            } else {
                setResult({ status: 'failed', error: error || 'Payment authentication failed' });
            }
            setIsProcessing(false);
        },
        [onPaymentComplete]
    ...
    // Process payment after token is received (MercadoPago)
    useEffect(() => {
        if (cardToken && cardPaymentMethodId && payerIdentification && isProcessing) {
            // For collectOnly (trials), just confirm the card is valid without charging
            if (collectOnly) {
    ...
    }, [cardToken, cardPaymentMethodId, payerIdentification, isProcessing, collectOnly, onPaymentComplete]);

    // Process Stripe payment after PaymentMethod is created
    useEffect(() => {
        console.log('[PaymentModal] Stripe effect check', {
        ...
        if (stripePaymentMethodId && isProcessing && !stripeClientSecret) {
            console.log('[PaymentModal] Stripe effect - conditions met');

            // For collectOnly (trials), just confirm the card is valid without charging
            // But save the PaymentMethod ID so it can be used for future charges
            if (collectOnly) {
                console.log('[PaymentModal] collectOnly=true, saving PaymentMethod for future use:', stripePaymentMethodId);

                // Save the PaymentMethod for future charges
                try {
                    ...
                    savePaymentMethod({
                        ...
                        type: 'card',
                        card: {
                            last4: '4242', // We don't have the actual last4 from Stripe here
                            brand: 'Visa',
                            expMonth: 12,
                    ...
                    console.log('[PaymentModal] Saved PaymentMethod for future use');
                } catch (saveError) {
                    console.warn('[PaymentModal] Failed to save PaymentMethod:', saveError);
                }

                const paymentResult: PaymentResult = {
                    status: 'succeeded',
                    paymentId: stripePaymentMethodId // Return the PaymentMethod ID
                };
                setResult(paymentResult);
                setIsProcessing(false);
                setStripePaymentMethodId(null);
            ...
            console.log('[PaymentModal] Calling processStripePayment');
            processStripePayment(stripePaymentMethodId);
        }
    }, [stripePaymentMethodId, isProcessing, stripeClientSecret, collectOnly, onPaymentComplete]);
        ...
        try {
            ...
            if (backendResult.data?.savedCard) {
                const newCard: SavedCard = {
                    id: backendResult.data.savedCard.id,
                    customerId: customerId,
                    lastFourDigits: backendResult.data.savedCard.lastFourDigits,
                    expirationMonth: backendResult.data.savedCard.expirationMonth,
                    expirationYear: backendResult.data.savedCard.expirationYear,
                    paymentMethodId: backendResult.data.savedCard.paymentMethodId,
                    paymentMethodName: backendResult.data.savedCard.paymentMethodName,
                    paymentMethodThumbnail: backendResult.data.savedCard.paymentMethodThumbnail
                };
    ...
    // Process Stripe payment with PaymentMethod
    const processStripePayment = async (paymentMethodId: string) => {
        console.log('[PaymentModal] processStripePayment called', { paymentMethodId, amount, currency, customerId });

        if (!billing) {
            console.log('[PaymentModal] No billing instance!');
            return;
        }

        try {
            const sessionId = backendSessionId || getBackendSessionId();
            console.log('[PaymentModal] Using sessionId:', sessionId);

            const customerData = await billing.customers.get(customerId);
            console.log('[PaymentModal] Customer data:', customerData);

            console.log('[PaymentModal] Calling processPaymentInBackend...');
            const backendResult = await processPaymentInBackend(sessionId, {
                customerId,
                amount,
                currency,
            ...
            console.log('[PaymentModal] Backend result:', backendResult);

            // Check if 3DS is required
            if (backendResult.data?.status === 'requires_action' && backendResult.data?.clientSecret) {
                setStripeClientSecret(backendResult.data.clientSecret);
                return; // Wait for 3DS confirmation in StripeCardForm
            }
            ...
            // Record the payment in frontend storage
            if (paymentResult.status === 'succeeded' && backendResult.data?.id) {
                try {
                    await billing.payments.record({
                        id: backendResult.data.id,
                        ...
                        providerPaymentId: backendResult.data.id,
                        provider: paymentMode,
                        metadata: { source: 'playground', paymentMethod: 'stripe_card' }
                    });
                } catch (recordError) {
                    console.warn('Failed to record payment in frontend storage:', recordError);
                }
    ...
    const handleProcessPayment = async () => {
        ...
        console.log('[PaymentModal] handleProcessPayment called', {
            useStripeCardForm,
            useMPCardForm,
            selectedSavedCard,
            collectOnly,
        ...
        if (useMPCardForm && selectedSavedCard) {
            console.log('[PaymentModal] Using MercadoPago saved card flow');
        ...
        if (useMPCardForm && !selectedSavedCard) {
            console.log('[PaymentModal] Using MercadoPago new card flow');
            setTriggerTokenization(true);
            // The actual payment will be processed in processPaymentWithToken after token is received
            return;
        }

        // For Stripe with new card, trigger PaymentMethod creation
        if (useStripeCardForm && !selectedSavedCard) {
            console.log('[PaymentModal] Using Stripe card form flow');
            setTriggerStripePaymentMethod(true);
            // The actual payment will be processed in processStripePayment after PaymentMethod is created
            return;
        }

        console.log('[PaymentModal] Using fallback/mock flow');
        ...
        try {
            if (collectOnly) {
                ...
                const paymentResult: PaymentResult = {
                    ...
                    error: payment.status === 'failed' ? (payment.failureMessage ?? t('subscriptions.payment.genericError')) : undefined
                };
                setResult(paymentResult);
                ...
                if (paymentResult.status === 'succeeded' && saveCardForFuture && !selectedSavedCard) {
                    try {
                        ...
                        const brand =
                            firstDigit === '4' ? 'Visa' : firstDigit === '5' ? 'Mastercard' : firstDigit === '3' ? 'Amex' : 'Card';
            ...
            <div className="modal-content max-w-md" onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                    <h3 className="text-lg font-semibold" style={{ color: 'var(--color-text)' }}>
                        {collectOnly ? t('subscriptions.payment.titleCollect') : t('subscriptions.payment.title')}
                    </h3>
                ...
                <div className="modal-body space-y-4">
                        ...
                        >
                            {isBackendConnected ? (
                                ...
                                <>
                                    ...
                                    <span className="text-yellow-400">{backendError || t('subscriptions.payment.backendFallback')}</span>
                                </>
                            )}
                        </div>
                    )}
                        ...
                        >
                            {result.status === 'succeeded' ? (
                                <>
                                    ...
                                    <div>
                                        <p className="font-medium text-green-400">{t('subscriptions.payment.success')}</p>
                                        {result.paymentId && <p className="text-xs text-green-400/70 font-mono">{result.paymentId}</p>}
                                    </div>
                                </>
                            ) : (
                                <>
                                    ...
                                    <div>
                                        <p className="font-medium text-red-400">{t('subscriptions.payment.failed')}</p>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-2">
                                        {savedCards.map((card) => (
                                            ...
                                            >
                                                {isMercadoPagoCard(card) && card.paymentMethodThumbnail ? (
                                                    <img
                                                ...
                                                <div className="flex-1 text-left">
                                                    <p className="text-sm font-medium text-white">
                                                        {(isMercadoPagoCard(card) ? card.paymentMethodName : card.brand) || 'Card'} ****
                                                        {card.lastFourDigits}
                                                    </p>
                                                    <p className="text-xs text-gray-400">
                                                        {t('subscriptions.payment.expires', 'Expires')}{' '}
                                                        {card.expirationMonth?.toString().padStart(2, '0')}/{card.expirationYear}
                                                    </p>
                                                </div>
                                                {selectedSavedCard === card.id && <CheckCircle className="h-5 w-5 text-blue-400" />}
                                            </button>
                                        ))}
                                        ...
                                        >
                                            ...
                                            {selectedSavedCard === null && <CheckCircle className="h-5 w-5 text-blue-400 ml-auto" />}
                                        </button>
                                    </div>
                    ...
                    {/* Stripe Card Form - Show when Stripe backend connected */}
                    {!result && useStripeCardForm && (
                        <div className="space-y-4">
                            <StripeCardForm
                                amount={amount}
                                onPaymentMethodCreated={handleStripePaymentMethodCreated}
                                onError={handleTokenError}
                                triggerPaymentMethod={triggerStripePaymentMethod}
                                onPaymentMethodComplete={handleStripePaymentMethodComplete}
                    ...
                    {/* Test Card Selection - Show for mock or without backend */}
                    {!result && !useMPCardForm && !useStripeCardForm && (
                        <div className="space-y-4">
                            {/* Loading saved cards */}
                            {isLoadingCards && (
                                <div className="flex items-center justify-center p-4">
                                    ...
                                    <div className="space-y-2">
                                        {savedCards.map((card) => (
                                            <button
                                                ...
                                                style={{
                                                    backgroundColor:
                                                        selectedSavedCard === card.id ? 'var(--color-accent-low)' : 'var(--color-surface)',
                                                    borderColor:
                                                        selectedSavedCard === card.id ? 'var(--color-accent)' : 'var(--color-border)'
                                                }}
                                            >
                                        ...
                                        <button
                                            ...
                                            style={{
                                                backgroundColor:
                                                    selectedSavedCard === null ? 'var(--color-accent-low)' : 'var(--color-surface)',
                                                ...
                                                {testCards.map((card, index) => {
                                                    ...
                                                    return (
                                                        <option key={`${card.value}-${index}`} value={index.toString()}>
                                                            {statusIcon} {label} - {card.value}
                                                            {extra}
                                                        </option>
                                                    );
                                                })}
                                                <option value="custom">✎ {t('subscriptions.payment.customCard')}</option>
                                            </select>
                                            <ChevronDown
                                    ...
                                    {selectedCard !== 'custom' && (
                                        ...
                                        >
                                            ...
                                            <span style={{ color: 'var(--color-text)' }}>
                                                {testCards[Number.parseInt(selectedCard, 10)]?.value}
                                            </span>
                                            {testCards[Number.parseInt(selectedCard, 10)]?.extra && (
                                                <span style={{ color: 'var(--color-text-muted)' }}>
                                                    {' '}
                                                    ({testCards[Number.parseInt(selectedCard, 10)]?.extra})
                                                </span>
                    ...
                    {result?.status === 'failed' ? (
                        ...
                        <>
                            ...
                            <button
                                ...
                                disabled={
                                    isProcessing ||
                                    (selectedCard === 'custom' && !customCardNumber) ||
                                    (useMPCardForm && selectedSavedCard !== null && savedCardCvv.length < 3)
                                }
                            >
                                {isProcessing ? (
                                    <>
