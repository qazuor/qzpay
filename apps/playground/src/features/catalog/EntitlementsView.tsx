import { AlertCircle, Check, Pencil, Plus, Shield, Trash2, X } from 'lucide-react';
import { useState } from 'react';
import { useTranslation } from 'react-i18next';
import { EmptyState } from '../../components/ui/EmptyState';
import { PageHeader } from '../../components/ui/PageHeader';
import { type EntitlementDefinition, useCatalogStore } from '../../stores/catalog.store';
...
export function EntitlementsView() {
    ...
    const { entitlementDefinitions, plans, addEntitlementDefinition, updateEntitlementDefinition, deleteEntitlementDefinition } =
        useCatalogStore();
    ...
    if (!isInitialized) {
        return <EmptyState icon={Shield} title={t('common.billingNotInitialized')} description={t('common.billingNotInitializedDesc')} />;
    }
        ...
        <div className="max-w-4xl space-y-6">
            ...
            {entitlementDefinitions.length === 0 ? (
                <EmptyState
                    ...
                    tips={[t('entitlements.tips.tip1'), t('entitlements.tips.tip2'), t('entitlements.tips.tip3')]}
                />
            ) : (
                        ...
                        <div className="space-y-2 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                            ...
                            <ul className="list-disc list-inside ml-2 space-y-1">
                                <li>
                                    ...
                                    - {t('entitlements.about.apiAccess')}
                                </li>
                                <li>
                                    <code className="font-mono text-xs" style={{ color: 'var(--color-accent)' }}>
                                        priority_support
                                    </code>{' '}
                                    - {t('entitlements.about.prioritySupport')}
                                </li>
                                <li>
                                    <code className="font-mono text-xs" style={{ color: 'var(--color-accent)' }}>
                                        advanced_analytics
                                    </code>{' '}
                                    - {t('entitlements.about.advancedAnalytics')}
                                </li>
                                <li>
                                    <code className="font-mono text-xs" style={{ color: 'var(--color-accent)' }}>
                                        custom_integrations
                                    </code>{' '}
                                    - {t('entitlements.about.customIntegrations')}
                                </li>
                            </ul>
                        </div>
                    </div>
