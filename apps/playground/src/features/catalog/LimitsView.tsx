import { AlertCircle, Check, Infinity, Pencil, Plus, Sliders, Trash2, X } from 'lucide-react';
import { useState } from 'react';
import { useTranslation } from 'react-i18next';
import { EmptyState } from '../../components/ui/EmptyState';
import { PageHeader } from '../../components/ui/PageHeader';
import { type LimitDefinition, useCatalogStore } from '../../stores/catalog.store';
...
export function LimitsView() {
    ...
    const { limitDefinitions, plans, addLimitDefinition, updateLimitDefinition, deleteLimitDefinition } = useCatalogStore();
    ...
    if (!isInitialized) {
        return <EmptyState icon={Sliders} title={t('common.billingNotInitialized')} description={t('common.billingNotInitializedDesc')} />;
    }
        ...
        <div className="max-w-4xl space-y-6">
            ...
            {showForm && (
                <div className="card p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button type="button" onClick={handleCancel} className="p-1 rounded hover:bg-opacity-10 hover:bg-white">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        onChange={(e) => setFormData({ ...formData, defaultValue: Number.parseInt(e.target.value) || 0 })}
            ...
            {limitDefinitions.length === 0 ? (
                <EmptyState
                    ...
                    tips={[t('limits.tips.tip1'), t('limits.tips.tip2'), t('limits.tips.tip3')]}
                />
            ) : (
                    ...
                    <div>
                        ...
                        <div className="space-y-2 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                            ...
                            <ul className="list-disc list-inside ml-2 space-y-1">
                                <li>
                                    <code className="font-mono text-xs" style={{ color: 'var(--color-accent)' }}>
                                        api_calls
                                    </code>{' '}
                                    - {t('limits.about.apiCalls')}
                                </li>
                                <li>
                                    <code className="font-mono text-xs" style={{ color: 'var(--color-accent)' }}>
                                        storage_gb
                                    </code>{' '}
                                    - {t('limits.about.storageGb')}
                                </li>
                                <li>
                                    <code className="font-mono text-xs" style={{ color: 'var(--color-accent)' }}>
                                        team_members
                                    </code>{' '}
                                    - {t('limits.about.teamMembers')}
                                </li>
                                <li>
                                    <code className="font-mono text-xs" style={{ color: 'var(--color-accent)' }}>
                                        projects
                                    </code>{' '}
                                    - {t('limits.about.projects')}
                                </li>
                            </ul>
                        </div>
                    </div>
