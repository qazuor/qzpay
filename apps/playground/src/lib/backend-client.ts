 ...
 */
export async function getCustomerCards(sessionId: string, customerId: string): Promise<BackendResponse<{ cards: BackendSavedCard[] }>> {
    try {
        const response = await fetch(`${BACKEND_URL}/api/customers/${customerId}/cards?sessionId=${sessionId}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        });
 ...
 * Delete a saved card
 */
export async function deleteCustomerCard(sessionId: string, customerId: string, cardId: string): Promise<BackendResponse<void>> {
    try {
        const response = await fetch(`${BACKEND_URL}/api/customers/${customerId}/cards/${cardId}?sessionId=${sessionId}`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
        });
