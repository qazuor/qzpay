...
export interface BillingFactoryConfig {
  ...
  onEvent?: <K extends keyof QZPayEventMap>(
    eventType: K,
 ...
 */
export async function createPlaygroundBilling(
  config: BillingFactoryConfig
): Promise<QZPayBilling> {
...
export function getBilling(): QZPayBilling {
  if (!billingInstance) {
    throw new Error(
      'Billing not initialized. Call createPlaygroundBilling first.'
    );
  }
  return billingInstance;
}
