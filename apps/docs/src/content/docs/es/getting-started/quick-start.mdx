...
5. **Crear una suscripción**
   ...
   const subscription = await billing.subscriptions.create({
     ...
     // Required: the product line this subscription belongs to.
     // Free-form and defined by your app — QZPay has no vocabulary
     // for it, and no default, so it can never answer for you.
     productDomain: 'saas'
   });
