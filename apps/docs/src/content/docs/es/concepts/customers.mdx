...
Busca un cliente por su ID del proveedor (almacenado en `providerCustomerIds`):

```typescript
// El ID del cliente del proveedor está almacenado en customer.providerCustomerIds.stripe
...
interface Customer {
  ...
  providerCustomerIds: Record<string, string>; // IDs del proveedor
  createdAt: Date;
  updatedAt: Date;
  deletedAt?: Date;
}
