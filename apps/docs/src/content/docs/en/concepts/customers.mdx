...
Look up a customer by their provider ID (stored in `externalIds`):

```typescript
// The externalId is stored in customer.externalIds.stripe
...
interface Customer {
  ...
  externalIds: Record<string, string>; // Provider IDs
  createdAt: Date;
  updatedAt: Date;
  deletedAt?: Date;
}
