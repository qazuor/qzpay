/**
 * Retry Utilities with Exponential Backoff for MercadoPago
 *
...
export function isRetriableError(error: unknown): boolean {
    ...
    if (error instanceof QZPayMercadoPagoError) {
        const retriableCodes: QZPayErrorCode[] = [
            ...
            QZPayErrorCode.PROCESSING_ERROR
        ];
    ...
    // Handle MercadoPago API errors with cause
    if (error && typeof error === 'object' && 'cause' in error && Array.isArray(error.cause)) {
        const causes = error.cause as Array<{ code?: string }>;
        const firstCause = causes[0];

        if (firstCause?.code) {
            const code = firstCause.code;

            // Retry on 429 and 5xx errors
            if (code === '429' || (code.startsWith('5') && code.length === 3)) {
                return true;
            }

            // Don't retry on validation, auth, or not found errors
